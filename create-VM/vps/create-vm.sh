...
sed \
  -e "s|\$USER|$USER|g" \
  -e "s|\$DEPOT-GIT|$DEPOT_GIT|g" \
  -e "s|\$ID-SSH-PUB|$(cat ~/.ssh/${ID_SSH}.pub)|g" \
  cloud-init-template.yaml > cloud-init.yaml
...
# On capture toute la sortie dans OUTPUT
...
# --- Extraction de l'IP (portable awk) ---
...
echo "ssh -i ~/.ssh/$ID_SSH -o StrictHostKeyChecking=no root@$VM_IP"
